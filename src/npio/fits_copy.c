...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3)
    {
 printf("Usage:  fits_copy inputfile outputfile\n");
 printf("\n");
 ...
 printf("Examples:\n");
 printf("\n");
 printf("fits_copy in.fit out.fit                   (simple file copy)\n");
 printf("fits_copy - -                              (stdin to stdout)\n");
 printf("fits_copy in.fit[11:50,21:60] out.fit      (copy a subimage)\n");
 printf("fits_copy iniraf.imh out.fit               (IRAF image to FITS)\n");
 printf("fits_copy in.dat[i512,512] out.fit         (raw array to FITS)\n");
 printf("fits_copy in.fit[events][pi>35] out.fit    (copy rows with pi>35)\n");
 printf("fits_copy in.fit[events][bin X,Y] out.fit  (bin an image) \n");
 printf("fits_copy in.fit[events][col x=.9*y] out.fit        (new x column)\n");
 printf("fits_copy in.fit[events][gtifilter()] out.fit       (time filter)\n");
 printf("fits_copy in.fit[2][regfilter(\"pow.reg\")] out.fit (spatial filter)\n");
 printf("\n");
